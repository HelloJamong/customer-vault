...
const fileExists = (p: string) => fsp.access(p).then(() => true).catch(() => false);
  ...
  async create(data: {
    ...
    try {
      ...
      const [customer, inspectionTarget] = await Promise.all([
        ...
        this.prisma.inspectionTarget.findUnique({
      ...
      const customerName = customer.name.replace(/[/\\?%*:|"<>]/g, '_');
      ...
      // 최종 디렉토리 생성
      await fsp.mkdir(finalDirectory, { recursive: true });
      ...
      // 임시 파일 존재 확인
      if (!await fileExists(data.tempFilepath)) {
