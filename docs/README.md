...
| 문서 | 설명 | 대상 |
|------|------|------|
| **[환경 설정 가이드](SETUP_GUIDE.md)** | 처음부터 끝까지 전체 시스템 설정 가이드 | 신규 관리자 |
...
#### 1️⃣ 처음 시스템을 설치하는 경우
1. [Docker 컨테이너 환경 구성](DOCKER_CONTAINER_GUIDE.md) - Docker 설치부터 시작
2. [환경 변수 참조](ENV_VARIABLES.md) - .env 파일 설정
3. [환경 설정 가이드](SETUP_GUIDE.md) - 전체 설정 단계

#### 2️⃣ .env 파일만 수정하고 싶은 경우
- [환경 변수 참조](ENV_VARIABLES.md) - 빠른 참조와 예시

#### 3️⃣ Docker 설정을 변경하고 싶은 경우
- [Docker 설정 가이드](DOCKER_GUIDE.md) - 포트, 리소스, 볼륨 등 수정

#### 4️⃣ 다른 서버에 배포하려는 경우
1. [환경 설정 가이드](SETUP_GUIDE.md) - 환경별 설정 방법
2. [Docker 컨테이너 환경 구성](DOCKER_CONTAINER_GUIDE.md) - 배포 과정
3. [환경 변수 참조](ENV_VARIABLES.md) - 운영 환경 설정
