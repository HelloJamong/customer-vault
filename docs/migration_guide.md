...
1. [버전별 마이그레이션 노트](#버전별-마이그레이션-노트)
   - [v26.05.02 업그레이드 가이드](#v260502-업그레이드-가이드)
...
> ✅ **v26.05.02는 하위 호환되는 변경만 포함합니다.**
> `.env` 수정 없이 이미지 교체만으로 업그레이드 가능합니다.

### 변경 사항 요약

| 항목 | 변경 내용 | 조치 필요 |
|------|-----------|-----------|
| DB 스키마 | `support_logs` 테이블에 `title` 컬럼 추가 | ✅ 자동 적용 (컨테이너 시작 시) |
| 지원 목록 UI | 목록 컬럼 `사용자 정보` → `제목` 으로 교체 | — |
| 지원 로그 폼 | `제목` 입력란 추가 (문의 내용 위) | — |

---

### 1단계: 이미지 교체 및 서비스 재시작
...
- 지원 목록 페이지에서 `제목` 컬럼 표시 확인
- 지원 로그 추가/수정 다이얼로그에서 `제목` 입력란 표시 확인
- 기존 지원 로그의 `제목` 항목이 빈값으로 표시되는 것 확인

> ℹ️ 기존 데이터의 `title` 컬럼은 `NULL`로 저장되며 UI에서 빈값으로 표시됩니다.
> 각 항목을 수정하여 제목을 입력할 수 있습니다.
