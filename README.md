...
- ✅ **오프라인 TOTP 2차 인증**: Google/Microsoft Authenticator 호환 OTP 사용
...
관리자는 시스템 설정에서 OTP를 전역으로 활성화할 수 있습니다. 활성화 후 각 사용자는 최초 로그인 시 Authenticator 앱으로 QR 코드를 등록해야 하며, 이후 로그인부터는 앱의 6자리 코드를 입력합니다. TOTP는 서버와 모바일 기기 사이에 외부 통신이 필요하지 않으므로 폐쇄망에서도 사용할 수 있습니다.

OTP를 분실한 경우 관리자 사용자 관리 화면의 **OTP 등록 초기화** 기능으로 해당 사용자의 등록을 초기화할 수 있습니다. 초기화 이력은 서비스 로그에 기록됩니다.
